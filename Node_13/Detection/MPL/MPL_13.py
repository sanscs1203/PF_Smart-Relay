# =============================================================================
# MLP (Multi-Layer Perceptron) - FAULT DETECTION MODEL
...
from sklearn.preprocessing import StandardScaler          # ← Requerido para MLP
from sklearn.inspection import permutation_importance     # ← Para feature importance
import json
import joblib
import warnings
from sklearn.exceptions import ConvergenceWarning
warnings.filterwarnings('ignore', category=ConvergenceWarning)  # MLP puede generar warnings
...
# Total: 5 × 2 × 3 × 2 = 60 combinations

print("=" * 60)
print("GRIDSEARCHCV CONFIGURATION")
print("=" * 60)
total_combinations = 5 * 2 * 3 * 2
...
print(f"\nMLP Configuration:")
print(f"  Solver: adam (default, good for small datasets)")
print(f"  Max iterations: 1000")
print(f"  Early stopping: True (10% validation split)")
print(f"  Random state: 242")
...
for i, (_, row) in enumerate(top5.iterrows()):
    ...
    # For MLP: total number of neurons in hidden layers
    hidden_layers = params.get('hidden_layer_sizes', (100,))
    if isinstance(hidden_layers, int):
        hidden_layers = (hidden_layers,)
    total_neurons = sum(hidden_layers)
    
    # Also consider alpha (higher alpha = more regularization = simpler effective model)
    alpha_val = params.get('alpha', 0.0001)
    
    # Complexity: more neurons and less regularization = more complex
    complexity = total_neurons / (alpha_val * 1000 + 1)
    ...
    print(f"  Architecture: {hidden_layers} ({total_neurons} neurons)")
...
print(f"  Architecture: {best_params_final.get('hidden_layer_sizes', (100,))}")
print(f"  Total neurons: {best_entry['total_neurons']}")
print(f"  Complexity: {best_entry['complexity']:.4f}")
print(f"  Tied models: {len(tied_models)}")
print(f"\n  Hyperparameters:")
for param, value in best_params_final.items():
    print(f"    {param}: {value}")

mlp_optimizado = best_model_final
...
y_pred = mlp_optimizado.predict(X_test_scaled)
y_proba = mlp_optimizado.predict_proba(X_test_scaled)[:, 1]

print("\n" + "=" * 60)
print("OPTIMIZED MLP - TEST SET RESULTS")
print("=" * 60)
...
print(f"\n  Architecture: {best_params_final.get('hidden_layer_sizes', (100,))}")
print(f"  Activation: {best_params_final.get('activation', 'relu')}")
print(f"  Alpha (L2 reg): {best_params_final.get('alpha', 0.0001)}")
print(f"  Learning rate: {best_params_final.get('learning_rate', 'constant')}")
print(f"\n  Training iterations: {mlp_optimizado.n_iter_}")
print(f"  Final loss: {mlp_optimizado.loss_:.6f}")

if hasattr(mlp_optimizado, 'best_loss_') and mlp_optimizado.best_loss_ is not None:
    print(f"  Best validation loss: {mlp_optimizado.best_loss_:.6f}")

# Layer information
print(f"\n  Network structure:")
print(f"    Input layer:  {X_train_scaled.shape[1]} neurons (features)")
for i, layer_size in enumerate(best_params_final.get('hidden_layer_sizes', (100,))):
    if isinstance(best_params_final.get('hidden_layer_sizes', (100,)), int):
        print(f"    Hidden layer: {best_params_final.get('hidden_layer_sizes', (100,))} neurons")
        break
    else:
...
print(f"  Architecture: {BEST_PARAMS.get('hidden_layer_sizes', (100,))}")
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
    ...
    # =========================================================================
    # IMPORTANT: Scale WITHIN each fold to prevent data leakage
    # =========================================================================
...
# --- Save trained model ---
model_path = 'Results/MLP_detection_13bus.pkl'
joblib.dump(mlp_optimizado, model_path)
print(f"  ✅ Model saved: {model_path}")

# --- Save scaler (REQUIRED for MLP deployment) ---
scaler_path = 'Results/MLP_scaler_13bus.pkl'
...
# --- Extract MLP architecture info ---
hidden_layers = best_params_final.get('hidden_layer_sizes', (100,))
if isinstance(hidden_layers, int):
    hidden_layers = (hidden_layers,)
total_neurons = sum(hidden_layers)
...
results_summary = {
    ...
    'hyperparameters': {
        ...
        'activation': best_params_final.get('activation', 'relu'),
        'alpha': best_params_final.get('alpha', 0.0001),
        'learning_rate': best_params_final.get('learning_rate', 'constant'),
        'solver': 'adam',
        'max_iter': 1000,
        'early_stopping': True,
        'validation_fraction': 0.1,
    ...
    'architecture': {
        ...
        'output_neurons': 2,
        'total_hidden_neurons': total_neurons,
        'n_hidden_layers': n_layers
    },
    'training_info': {
        'iterations': int(mlp_optimizado.n_iter_),
        'final_loss': float(mlp_optimizado.loss_),
        'best_loss': float(mlp_optimizado.best_loss_) if (hasattr(mlp_optimizado, 'best_loss_') and mlp_optimizado.best_loss_ is not None) else None,
        'converged': mlp_optimizado.n_iter_ < 1000
    },
...
results_path = 'Results/MLP_detection_13bus_summary.json'
with open(results_path, 'w') as f:
    json.dump(results_summary, f, indent=2)
print(f"  ✅ Summary saved: {results_path}")

# --- Save CV results as CSV ---
df_cv_def.to_csv('Results/MLP_cv_default.csv', index=False)
df_cv_opt.to_csv('Results/MLP_cv_optimized.csv', index=False)
print("  ✅ CV results saved: Results/MLP_cv_default.csv")
print("  ✅ CV results saved: Results/MLP_cv_optimized.csv")

# --- Save threshold sweep ---
df_sweep.to_csv('Results/MLP_threshold_sweep.csv', index=False)
print("  ✅ Threshold sweep saved: Results/MLP_threshold_sweep.csv")

# --- Save feature importance ---
df_importance.to_csv('Results/MLP_feature_importance.csv', index=False)
print("  ✅ Feature importance saved: Results/MLP_feature_importance.csv")
...
fig.suptitle('MLP (Multi-Layer Perceptron) - Fault Detection Model\n'
...
max_count = max(max(default_counts), max(optimal_counts), 1)
ax6.set_ylim(0, max_count * 1.3)
...
plt.savefig('Images/MLP_detection_dashboard.png', dpi=200, bbox_inches='tight')
print("  📊 Dashboard saved: Images/MLP_detection_dashboard.png")
...
fig2.suptitle('Probability Distribution Analysis - MLP Detection',
...
bp = ax_p2.boxplot(data_box, tick_labels=['No Fault', 'Fault'],
...
plt.tight_layout()
plt.savefig('Images/MLP_probability_analysis.png', dpi=150, bbox_inches='tight')
print("  📊 Probability analysis saved: Images/MLP_probability_analysis.png")

# =============================================================================
# FIGURE 3: MLP TRAINING HISTORY (Loss Curve)
# =============================================================================

fig3, ax_loss = plt.subplots(1, 1, figsize=(10, 5))
fig3.suptitle('MLP Training History', fontsize=14, fontweight='bold')

if hasattr(mlp_optimizado, 'loss_curve_'):
    epochs = range(1, len(mlp_optimizado.loss_curve_) + 1)
    ax_loss.plot(epochs, mlp_optimizado.loss_curve_, 'b-', linewidth=2, label='Training Loss')
    
    if hasattr(mlp_optimizado, 'validation_scores_') and mlp_optimizado.validation_scores_:
        ax_loss.plot(epochs, mlp_optimizado.validation_scores_, 'r-', linewidth=2, label='Validation Score')
    
    ax_loss.set_xlabel('Epoch', fontsize=11)
    ax_loss.set_ylabel('Loss', fontsize=11)
    ax_loss.set_title(f'Training Convergence (Final loss: {mlp_optimizado.loss_:.6f})', fontsize=12)
    ax_loss.legend(fontsize=10)
    ax_loss.grid(True, alpha=0.3)
    
    # Mark early stopping point if applicable
    if mlp_optimizado.n_iter_ < 1000:
        ax_loss.axvline(x=mlp_optimizado.n_iter_, color='green', linestyle='--', 
                        alpha=0.7, label=f'Early stop @ {mlp_optimizado.n_iter_}')
        ax_loss.legend(fontsize=10)
else:
    ax_loss.text(0.5, 0.5, 'Loss curve not available', ha='center', va='center',
                 fontsize=14, transform=ax_loss.transAxes)

plt.tight_layout()
plt.savefig('Images/MLP_training_history.png', dpi=150, bbox_inches='tight')
print("  📊 Training history saved: Images/MLP_training_history.png")
...
│ hidden_layer_sizes       │ {arch_str:<40} │
│ activation               │ {best_params_final.get('activation', 'relu'):<40} │
│ alpha (L2 reg)           │ {best_params_final.get('alpha', 0.0001):<40} │
│ learning_rate            │ {best_params_final.get('learning_rate', 'constant'):<40} │
│ solver                   │ adam                                     │
│ max_iter                 │ 1000                                     │
│ early_stopping           │ True                                     │
│ Decision threshold       │ {optimal_threshold:<40.2f} │
│ Scaling                  │ StandardScaler (fitted on train only)   │
│ Optimization method      │ GridSearchCV (60 combinations × 5 CV)   │
│ Selection criteria       │ AHP Score + Occam's razor tie-breaking  │
├──────────────────────────┼──────────────────────────────────────────┤
│ Training iterations      │ {mlp_optimizado.n_iter_:<40} │
│ Final loss               │ {mlp_optimizado.loss_:<40.6f} │
│ Converged                │ {str(mlp_optimizado.n_iter_ < 1000):<40} │
└──────────────────────────┴──────────────────────────────────────────┘
""")
...
{'='*70}
MLP (MULTI-LAYER PERCEPTRON) DETECTION MODEL - COMPLETE ✅
{'='*70}

  Architecture: {arch_str}
  Total hidden neurons: {total_neurons}
  Training iterations: {mlp_optimizado.n_iter_}
  Final loss: {mlp_optimizado.loss_:.6f}

  Files generated:
  │
  ├── Results/
  │   ├── MLP_detection_13bus.pkl          (trained model)
  │   ├── MLP_scaler_13bus.pkl             (StandardScaler - REQUIRED)
  │   ├── MLP_detection_13bus_summary.json (complete config & results)
  │   ├── MLP_cv_default.csv              (CV - default threshold)
  │   ├── MLP_cv_optimized.csv            (CV - optimized threshold)
  │   ├── MLP_threshold_sweep.csv         (all thresholds evaluated)
  │   └── MLP_feature_importance.csv      (permutation importance)
  │
  └── Images/
      ├── MLP_detection_dashboard.png     (7-panel summary)
      ├── MLP_probability_analysis.png    (probability distributions)
      └── MLP_training_history.png        (loss curve)
  ...
  ⚠️  IMPORTANT: For deployment, load BOTH model and scaler:
      model = joblib.load('Results/MLP_detection_13bus.pkl')
      scaler = joblib.load('Results/MLP_scaler_13bus.pkl')
