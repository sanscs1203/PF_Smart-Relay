...
--mode lab  (laboratory validation)
    ...
    Output: predictions CSV + validation report JSON comparing predictions
            ...
            The same scaler.pkl trained on simulation data is applied
...
In both modes the pipeline is identical:
    ...
    2. Load and apply scaler.pkl (transform only — never refit)
...
def main(config_path: str, mode: str, input_csv: str) -> None:
    ...
    3. Apply scaler.pkl (transform only).
    ...
    if mode == "lab":
        scaler = load_scaler(str(processed_dir / "scaler.pkl"))
