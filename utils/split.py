...
correct because both modules share the same 6 electrical features
...
def load_processed_csv(file_path: str, label_col: str) -> tuple[pd.DataFrame, pd.Series]:
    ...
    tuple[pd.DataFrame, pd.Series]
        (X, y) where X contains only FEATURE_COLS and y is the label.
    """
    path = Path(file_path)
    if not path.exists():
...
def process_module(
    ...
    # Load
    X, y = load_processed_csv(processed_csv, label_col)
    total = len(y)
...
def main(config_path: str) -> None:
    ...
    Steps:
      1. Load config.yaml
      2. Split + scale detection data  →  fit scaler on det_train
      3. Split + scale classification data  →  reuse same scaler
